...
func (c *Client) GetAutorizationUrl(endpointToken string) (string, error) {
	if endpointToken == "" || c.RedirectUri == "" {
		return "", errors.New("request token and redirect uri can't be nothing")
	}

	return fmt.Sprintf(host+endpointAutorization, endpointToken, c.RedirectUri), nil
}
...
func (c *Client) Add(urlToAdd string) error {
	if urlToAdd == "" {
		return errors.New("url can't be nothig")
	}
